...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
    ...
    /** 由ForkJoinPool和workers直接调用，需要是volatile的 */
    ...
    private int doInvoke() {
        ...
        return (s = doExec()) < 0 ? s : ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread)
            ? (wt = (ForkJoinWorkerThread)t).pool.awaitJoin(wt.workQueue, this, 0L) : externalAwaitDone();
    }
    ...
    private void reportException(int s) {
        rethrow((s & THROWN) != 0 ? getThrowableException() : new CancellationException());
    }
    ...
    /**
     *
     * getRawResult()是ForkJoinTask中的一个模板方法，分别被RecursiveAction和RecursiveTask实现，
     * 前者没有返回值，所以返回null，后者返回一个类型为V的result变量。
     * @date 2022/6/21 23:04
     * @param
     * @return V
     */
    public final V join() {
    ...
    private int doJoin() {
        ...
        // 如果status < 0表示任务已经完成，不用阻塞，直接返回。// 否则判断线程是否是工作线程
        return (s = status) < 0 ? s
    ...
    public final Throwable getException() {
        int s = status;
        return ((s & ABNORMAL) == 0 ? null : (s & THROWN) == 0 ? new CancellationException() : getThrowableException());
    }
